...
function* addToCartSaga({payload}) {
  ...
  if (!status) {
    ...
    const notification = message
      ? {
          message: 'Невозможно изменить количество',
          description: 'Количество на складе в магазине.',
        }
      : {
          message: `Вы не можете добавить в корзину ${product.get('name')}.`
        };
    ...
    showMessage({
      message: `"${product.get('name')}" в корзине.`,
      type: 'success',
    });
    yield put({
  ...
  try {
    ...
    if (!checkCoupon) {
      yield call(handleError, {
        message: 'Пожалуйста, проверьте код еще раз',
      });
    } else {
      yield put({
