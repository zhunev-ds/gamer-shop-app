...
class DemoConfig extends React.Component {
  ...
  handleSave = async () => {
    ...
    try {
      // Check WP website
      await fetch(url);
      const wp = await request(`${url}/wp-json`);
      if (!isObject(wp.data)) {
        showMessage({
      ...
      const rnlab = await request(`${url}/wp-json/rnlab-app-control/v1/settings`);
      ...
      // Check Woocommerce
      const woo = await request(`${url}/wp-json/wc/v3/products?consumer_key=${consumer_key}&consumer_secret=${consumer_secret}`);
